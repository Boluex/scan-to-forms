...
def save_upload(order, actor, data):
    ...
    if existing:
        if existing.sha256 != inspection["sha256"]:
            raise ValidationError(
                "This upload slot contains a different file. Remove it before replacing it."
            )
        return existing  # Stable slot key makes interrupted upload retries idempotent.
    editable = order.status == "UPLOADING" or (
        actor.is_staff and order.status in {"PAID", "NEEDS_REVIEW", "FAILED"}
    )
    if not editable:
    ...
    if order.service_type == "SYNTHETIC_DATA" and kind != "TEMPLATE":
        raise ValidationError(
            "Synthetic input is one blank questionnaire template, never respondent data."
        )
    ...
    if kind == "RESPONSE":
        ...
        if inspection["page_count"] > 1 or inspection["content_type"] == "application/pdf":
            if (
                ...
                or inspection["page_count"] != order.pages_per_respondent
                or response.pages.exists()
            ):
                raise ValidationError(
                    "Upload one complete PDF per respondent, with the stated page count and no existing pages."
                )
            page_number = None
        else:
            ...
            if response.pages.filter(assigned_template_page_number=page_number).exists():
                raise ValidationError(
                    "This page slot is occupied; remove the incorrect upload first."
                )
        if (
            upload_summary(order)["uploaded_pages"] + inspection["page_count"]
        ...
        expected = (
            ...
            else order.pages_per_respondent
        )
    ...
    document = UploadedDocument.objects.create(
        ...
        questionnaire=order.questionnaire if kind == "TEMPLATE" else None,
        batch=order.response_batch if kind == "RESPONSE" else None,
        response=response,
        document_type=kind,
        file=upload,
    ...
    log_event(
        ...
        actor,
        "file_uploaded",
        metadata={"document_id": str(document.pk), "kind": kind, "pages": document.page_count},
    )
    return document
...
def delete_upload(order, document, actor):
    ...
    if document.order_id != order.id or order.status not in {"UPLOADING", "AWAITING_PAYMENT"}:
        raise ValidationError(
            "Only this order’s unpaid uploads can be removed here. Paid corrections require an operator."
        )
