    ...
    def validate(self, attrs):
        if attrs["service_type"] == "DIGITIZATION":
            if (
                ...
                or attrs["synthetic_response_count"]
            ):
                raise serializers.ValidationError(
                    "Choose a supported respondent count; digitization has no synthetic responses."
                )
            ...
            if (
                attrs["respondent_count"]
                or not 1 <= attrs["synthetic_response_count"] <= settings.MAX_SYNTHETIC_RESPONSES
            ):
                raise serializers.ValidationError(
                    "Synthetic orders describe one template and a synthetic response count, not human respondents."
                )
            if not attrs.get("expected_page_count"):
                raise serializers.ValidationError(
                    "Enter the number of pages in the one blank questionnaire."
                )
            attrs["pages_per_respondent"] = 1
        if attrs["questions"] and len(attrs["questions"]) != attrs["question_count"]:
            raise serializers.ValidationError(
                "Enter exactly the stated number of questions, or leave schema preparation to the operator."
            )
        attrs["google_form_id"] = (
            normalize_form_id(attrs["google_form_url"]) if attrs["google_form_url"] else ""
        )
        return attrs
...
class OrderSerializer(serializers.ModelSerializer):
    ...
    def get_uploads_summary(self, obj) -> dict:
        return upload_summary(obj)

    def get_bank(self, obj) -> dict | None:
        return bank_details() if obj.status in {"AWAITING_PAYMENT", "PAYMENT_SUBMITTED"} else None

    def get_payment_available(self, obj) -> bool:
    ...
    def get_classification(self, obj) -> str:
        return (
            "SYNTHETIC TEST DATA — NOT HUMAN RESEARCH RESPONSES"
            if obj.service_type == "SYNTHETIC_DATA"
            else "DIGITIZED HUMAN RESPONSES"
        )

    def get_processing_mode(self, obj) -> str:
...
class UploadSerializer(serializers.Serializer):
    ...
    grouping_mode = serializers.ChoiceField(
        choices=["BULK_ORDERED", "MANUAL_RESPONSE", "PDF_PER_RESPONSE"], default="BULK_ORDERED"
    )
...
class SchemaSerializer(serializers.Serializer):
    ...
    def validate_questions(self, value):
        if len({q["key"] for q in value}) != len(value) or len(
            {q["position"] for q in value}
        ) != len(value):
            raise serializers.ValidationError("Question keys and positions must be unique.")
        for question in value:
            options = question.get("options", [])
            if len({o["key"] for o in options}) != len(options) or len(
                {o["position"] for o in options}
            ) != len(options):
