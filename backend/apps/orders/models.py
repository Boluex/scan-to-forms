...
class Order(models.Model):
    ...
    questionnaire = models.OneToOneField(
        "questionnaires.Questionnaire", on_delete=models.PROTECT, related_name="order"
    )
    response_batch = models.OneToOneField(
    ...
    bot_run = models.OneToOneField(
        "botlab.BotRun", on_delete=models.PROTECT, null=True, blank=True, related_name="order"
    )
    apps_script_job = models.OneToOneField(
        "googleforms.AppsScriptJob",
        on_delete=models.PROTECT,
        null=True,
        blank=True,
        related_name="order",
    ...
    class Meta:
        ...
        constraints = [
            models.CheckConstraint(
                condition=models.Q(amount_ngn__gt=0), name="order_positive_amount"
            ),
            models.CheckConstraint(
                condition=(
                    models.Q(
                        service_type="DIGITIZATION",
                        respondent_count__gt=0,
                        synthetic_response_count=0,
                        response_batch__isnull=False,
                    )
                    | models.Q(
                        service_type="SYNTHETIC_DATA",
                        respondent_count=0,
                        synthetic_response_count__gt=0,
                        response_batch__isnull=True,
                    )
                ),
                name="order_service_consistency",
            ),
            models.CheckConstraint(
                condition=(
                    models.Q(
                        payment_status="VERIFIED",
                        payment_verified_at__isnull=False,
                        payment_verified_by__isnull=False,
                    )
                    | (
                        ~models.Q(payment_status="VERIFIED")
                        & models.Q(
                            payment_verified_at__isnull=True, payment_verified_by__isnull=True
                        )
                    )
                ),
