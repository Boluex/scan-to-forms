...
def log_event(order, actor, action, *, before='', note='', metadata=None):
    OrderEvent.objects.create(order=order, actor=actor, action=action, from_status=before, to_status=order.status, note=note, metadata=metadata or {})
    record_audit(actor=actor, action=f'order.{action}', target=order, metadata={'reference': order.reference, **(metadata or {})})


def notify(order, kind, title, message=''):
    Notification.objects.create(user=order.user, kind=kind, title=title, message=message, data={'order_reference': order.reference})
...
def transition(order, target, actor, *, note=''):
    ...
    if target in {'PAID', 'QUEUED', 'PROCESSING', 'READY', 'COMPLETED'} and order.payment_status != 'VERIFIED':
...
def price_for(service, count):
    field = 'DIGITIZATION_PRICE_PER_RESPONDENT_NGN' if service == 'DIGITIZATION' else 'SYNTHETIC_PRICE_PER_RESPONSE_NGN'
    try:
    ...
    return amount, {'model': 'per_response', 'currency': 'NGN', 'unit_rate': str(rate), 'quantity': count}


def bank_details():
    return {'bank_name': settings.BANK_NAME, 'account_name': settings.BANK_ACCOUNT_NAME, 'account_number': settings.BANK_ACCOUNT_NUMBER}
...
def create_order(user, data):
    ...
    count = data['respondent_count'] if service == 'DIGITIZATION' else data['synthetic_response_count']
    amount, snapshot = price_for(service, count)
    questionnaire = Questionnaire.objects.create(owner=user, title=data['title'])
    pages = data['pages_per_respondent'] if service == 'DIGITIZATION' else data['expected_page_count']
    version = QuestionnaireVersion.objects.create(questionnaire=questionnaire, version_number=1, expected_page_count=pages)
    for i, text in enumerate(questions, 1):
        Question.objects.create(version=version, key=f'q{i}', position=i, text=text, type='SHORT_TEXT')
    batch = None
    if service == 'DIGITIZATION':
        data['expected_page_count'] = count * pages
        batch = ResponseBatch.objects.create(owner=user, questionnaire_version=version, name=data['title'])
        Response.objects.bulk_create([Response(batch=batch, sequence=i, respondent_reference=f'R-{i:04d}', expected_page_count=pages) for i in range(1, count + 1)])
    order = Order.objects.create(user=user, questionnaire=questionnaire, response_batch=batch, amount_ngn=amount, pricing_snapshot=snapshot, **data)
    order.reference = f'STF-{order.pk:06d}'
    order.save(update_fields=['reference'])
    log_event(order, user, 'created', metadata={'classification': 'SYNTHETIC TEST DATA' if service == 'SYNTHETIC_DATA' else 'DIGITIZED HUMAN RESPONSES'})
...
def upload_summary(order):
    ...
    return {'expected_pages': order.expected_page_count, 'uploaded_pages': uploaded, 'template_pages': sum(d.page_count for d in templates), 'expected_respondents': order.respondent_count, 'duplicate_files': duplicate, 'incomplete_respondents': missing, 'complete': complete}
...
def submit_for_payment(order, actor):
    ...
    if not upload_summary(order)['complete']:
        raise ValidationError('Upload the expected pages and resolve duplicate/missing pages before payment.')
...
def claim_payment(order, actor, sender='', reference=''):
    ...
    log_event(order, actor, 'payment_claimed')
    return order
...
def require_paid(order):
    if order.payment_status != 'VERIFIED' or not order.payment_verified_at or order.status == 'CANCELLED':
...
def prepare_schema(order, actor, questions):
    ...
    if order.status not in {'PAID', 'NEEDS_REVIEW', 'FAILED'} or version.questions.filter(answers__isnull=False).exists() or order.bot_run_id:
        raise ValidationError('Prepare the schema before processing. Existing reviewed data cannot be replaced.')
...
def start_processing(order, actor):
    ...
    if order.service_type == 'SYNTHETIC_DATA':
        ...
        transition(order, 'NEEDS_REVIEW', actor, note='Awaiting operator transcription and page inspection.')
    ...
    notify(order, 'PROCESSING_STARTED', f'{order.reference}: processing started', 'We will notify you when your order is ready.')
    return order
...
def dispatch_jobs(order_id, jobs, task):
    try:
        ...
        if order.status not in {'READY', 'COMPLETED'}:
            ...
            log_event(order, None, 'queue_failed', note='Broker delivery failed; operator may retry.')
            notify(order, 'NEEDS_ATTENTION', f'{order.reference}: processing needs attention')


@transaction.atomic
def invalidate_batch_delivery(batch_id):
    for order in Order.objects.select_for_update().filter(response_batch_id=batch_id, status__in=['READY', 'COMPLETED']):
        order.apps_script_job = None
        order.ready_at = None
        order.completed_at = None
        transition(order, 'NEEDS_REVIEW', None, note='Source data changed; previous delivery invalidated.')
...
def readiness_errors(order):
    ...
    if order.service_type == 'DIGITIZATION':
        ...
        if not order.bot_run_id or order.bot_run.status != 'COMPLETED' or order.bot_run.responses.count() != order.synthetic_response_count:
        ...
        if order.apps_script_job_id:
            errors.append('Synthetic Google submission is disabled until classification survives submission.')
    return errors
...
def mark_ready(order, actor):
    ...
    notify(order, 'ORDER_READY', f'{order.reference}: your order is ready', 'Open your order to retrieve your results.')
    return order
...
def generate_synthetic(order, actor):
    ...
    if order.service_type != 'SYNTHETIC_DATA' or order.status != 'PROCESSING' or order.bot_run_id:
        raise ValidationError('Start the synthetic order first; an attached run cannot be replaced here.')
    run = BotRun.objects.create(owner=order.user, questionnaire_version=order.questionnaire.versions.first(), requested_responses=order.synthetic_response_count, seed=secrets.randbits(63))
    order.bot_run = run
    order.save(update_fields=['bot_run', 'updated_at'])
    log_event(order, actor, 'synthetic_generation_requested', metadata={'classification': 'SYNTHETIC TEST DATA'})
    transaction.on_commit(lambda: dispatch_synthetic(order.pk, str(run.pk), generate_synthetic_responses))
    return order
...
def dispatch_synthetic(order_id, run_id, task):
    try:
        ...
        notify(order, 'NEEDS_ATTENTION', f'{order.reference}: synthetic preparation needs attention')
...
def document_finished(document):
    if not document.order_id:
        return
    order = Order.objects.select_for_update().get(pk=document.order_id)
    states = list(order.uploads.filter(document_type='RESPONSE').values_list('ocr_job__status', flat=True))
    if states and not any(s in (None, 'QUEUED', 'PROCESSING') for s in states) and order.status in {'PROCESSING', 'QUEUED', 'FAILED'}:
        transition(order, 'NEEDS_REVIEW', None, note='Operator review is required before any customer result is released.')
