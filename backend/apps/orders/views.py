...
class OrderViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        ...
        qs = Order.objects.select_related(
            "user", "questionnaire", "response_batch", "bot_run", "apps_script_job"
        )
    ...
    def configuration(self, request):
        return APIResponse(
            {
                "digitization_rate_ngn": settings.DIGITIZATION_PRICE_PER_RESPONDENT_NGN,
                "synthetic_rate_ngn": settings.SYNTHETIC_PRICE_PER_RESPONSE_NGN,
                "payment_available": services.payment_available(),
                "max_respondents": settings.MAX_ORDER_RESPONDENTS,
                "max_pages": settings.MAX_ORDER_PAGES,
                "max_synthetic_responses": settings.MAX_SYNTHETIC_RESPONSES,
                "test_deployment": settings.TEST_DEPLOYMENT,
    ...
    def upload_metadata(document):
        return {
            "id": str(document.pk),
            "upload_key": document.upload_key,
            "filename": document.original_filename,
            "page_count": document.page_count,
            "kind": document.document_type,
            "status": document.status,
            "respondent_sequence": document.response.sequence if document.response_id else None,
        }

    @extend_schema(parameters=[OpenApiParameter("document_id", str, OpenApiParameter.PATH)])
    ...
    @extend_schema(parameters=[OpenApiParameter("document_id", str, OpenApiParameter.PATH)])
    @decorators.action(
        detail=True, methods=["get"], url_path="uploads/(?P<document_id>[^/.]+)/file"
    )
    ...
    def payment_claim(self, request, reference=None):
        ...
        return self.result(
            services.claim_payment(
                self.get_object(), request.user, data["sender_name"], data["reference"]
            )
        )
    ...
    def questionnaire_schema(self, request, reference=None):
        ...
        return APIResponse(
            QuestionnaireVersionSerializer(order.questionnaire.versions.first()).data
        )
        ...
        rows = [
            {
                "id": str(r.pk),
                "sequence": r.sequence,
                "reference": r.respondent_reference,
                "status": r.status,
                "expected_pages": r.expected_page_count,
                "uploaded_pages": r.pages.count(),
                "missing_pages": sorted(
                    set(range(1, r.expected_page_count + 1))
                    - {p.assigned_template_page_number for p in r.pages.all()}
                ),
                "failed_pages": sum(p.processing_status == "FAILED" for p in r.pages.all()),
    ...
    def operator_detail(self, request, reference=None):
        ...
        return APIResponse(
            {
                "response_batch": str(order.response_batch_id) if order.response_batch_id else None,
                "questionnaire": str(order.questionnaire_id),
                "bot_run": str(order.bot_run_id) if order.bot_run_id else None,
                "apps_script_job": str(order.apps_script_job_id)
                if order.apps_script_job_id
                else None,
                "notes": order.operator_notes,
                "readiness_errors": services.readiness_errors(order)[:30],
                "events": list(
                    order.events.values("action", "from_status", "to_status", "note", "created_at")
                ),
            }
        )
    ...
    def manual_page_reviewed(self, request, reference=None):
        ...
        if settings.PROCESSING_MODE != "manual" or order.status != "NEEDS_REVIEW":
            raise ValidationError(
                "Manual page inspection is available only during manual transcription review."
            )
        page = get_object_or_404(
            DocumentPage,
            pk=request.data.get("page_id"),
        ...
        services.log_event(
            order, request.user, "page_manually_inspected", metadata={"page_id": str(page.pk)}
        )
        return APIResponse(
            {
                "detail": "Page inspection recorded. Answers still require manual transcription and approval."
            }
        )
    ...
    def attach_bot_run(self, request, reference=None):
        ...
        services.require_paid(order)
        if order.service_type != "SYNTHETIC_DATA" or order.status not in {
            "PROCESSING",
            "NEEDS_REVIEW",
            "FAILED",
        }:
        ...
        run = get_object_or_404(
            BotRun,
            pk=request.data.get("bot_run"),
            owner=order.user,
            questionnaire_version__questionnaire=order.questionnaire,
            status="COMPLETED",
        )
        if (
            run.responses.count() != order.synthetic_response_count
            or Order.objects.filter(bot_run=run).exclude(pk=order.pk).exists()
        ):
    ...
    def synthetic_responses(self, request, reference=None):
        ...
        if request.method == "POST":
            ...
            services.log_event(
                order,
                request.user,
                "synthetic_row_corrected",
                metadata={"sequence": row.sequence, "classification": "SYNTHETIC TEST DATA"},
            )
    ...
    def prepare_script(self, request, reference=None):
        ...
        order = Order.objects.select_for_update().get(pk=self.get_object().pk)
        services.require_paid(order)
        if order.service_type != "DIGITIZATION" or order.status not in {
            "PROCESSING",
            "NEEDS_REVIEW",
        }:
            raise ValidationError(
                "Only digitization orders under review support Apps Script delivery. Synthetic submission is blocked."
            )
    ...
    def complete(self, request, reference=None):
        ...
        services.transition(
            ...
            "COMPLETED",
            request.user,
            note="Delivery acknowledged; Google execution is not remotely verified.",
        )
        return self.result(order)
    ...
    def script(self, request, reference=None):
        ...
        if request.query_params.get("download") == "1":
            response = HttpResponse(
                order.apps_script_job.script, content_type="text/javascript; charset=utf-8"
            )
            response["Content-Disposition"] = f'attachment; filename="{order.reference}.gs"'
            return response
        return APIResponse(
            {
                "script": order.apps_script_job.script,
                "classification": order.apps_script_job.data_classification,
            }
        )
    ...
    def export(self, request, reference=None):
        ...
        services.log_event(
            ...
            request.user,
            "result_exported",
            metadata={"format": fmt, "service": order.service_type},
        )
        response = HttpResponse(
            payload,
        ...
        response["Content-Disposition"] = (
            f'attachment; filename="{order.reference}-{order.service_type.lower()}.{fmt}"'
        )
        return response
