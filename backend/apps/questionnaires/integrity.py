...
def answer_error(answer):
    ...
    if q.type in (Question.Type.SHORT_TEXT, Question.Type.PARAGRAPH) and not isinstance(value, str):
        return "Supply a text answer."
    ...
    if q.type == Question.Type.MULTIPLE_CHOICE:
        if not isinstance(value, list) or any(
            not isinstance(v, str) or v not in options for v in value
        ):
    ...
    if q.type in (Question.Type.NUMBER, Question.Type.LINEAR_SCALE, Question.Type.LIKERT):
        if q.type == Question.Type.LIKERT and options:
            return None if str(value) in options else "Choose one configured Likert option."
        try:
...
def final_errors(response, *, require_confirmation=True):
    ...
    if require_confirmation and (
        response.status != Response.Status.CONFIRMED or not response.confirmed_at
    ):
    ...
    if sorted(p.assigned_template_page_number or 0 for p in pages) != list(
        range(1, response.expected_page_count + 1)
    ):
    ...
    answers = list(
        response.answers.select_related("question").prefetch_related("question__options")
    )
    ...
    for answer in answers:
        if answer.review_status not in (
            Answer.ReviewStatus.APPROVED,
            Answer.ReviewStatus.CORRECTED,
        ):
...
def final_responses(batch):
    return [
        r
        for r in batch.responses.filter(
            status=Response.Status.CONFIRMED, confirmed_at__isnull=False
        )
        .select_related("batch__questionnaire_version")
        .prefetch_related("pages", "answers__question__options")
        .order_by("sequence", "created_at")
        if not final_errors(r)
    ]
